...
// --- General LLM Response (for non-weather queries) ---
/**
 * Provides a general AI response when no location is detected.
 * @param {string} userQuery The user's input text.
...
async function getGeneralAIResponse(userQuery, theme) {
  console.log(`Getting general AI response for theme: ${theme}`);
...
// --- Weather API Functions (Unchanged) ---
  ...
  try {
    ...
    if (!locationInfo || !locationInfo.location) {
      ...
      if (theme === "weather") {
        ...
        const weatherResponse = await getGeneralAIResponse(
        ...
        const generalResponse = await getGeneralAIResponse(userQuery, theme);
    ...
    return `I'm sorry, I ran into a problem: ${err.message}. Please try again with a different query.`;
  }
}

// --- Enhanced Example Usage ---
async function runExamples() {
  console.log("--- Running Example 1: Weather Query with Location ---");
  const prompt1 = "What kind of jacket should I wear for a night out in San Francisco?";
  await respondToUserQuery(prompt1);
  
  console.log("\n\n--- Running Example 2: Japanese Weather Query ---");
  const prompt2 = "今日の東京の天気はどうですか？";
  await respondToUserQuery(prompt2);

  console.log("\n\n--- Running Example 3: Weather Query without Location ---");
  const prompt3 = "Is it going to rain today?";
  await respondToUserQuery(prompt3);

  console.log("\n\n--- Running Example 4: Non-Weather Query (Travel) ---");
  const prompt4 = "What are the best places to visit in Japan?";
  await respondToUserQuery(prompt4);

  console.log("\n\n--- Running Example 5: Non-Weather Query (Fashion) ---");
  const prompt5 = "What are the latest fashion trends this season?";
  await respondToUserQuery(prompt5);

  console.log("\n\n--- Running Example 6: General Conversation ---");
  const prompt6 = "Hello! How are you doing today?";
  await respondToUserQuery(prompt6);
}

// runExamples();

module.exports = { respondToUserQuery, extractTheme };